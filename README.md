...
- `repository_dispatch` (`proto_release`) or **Run workflow** with `proto_ref`
- Checks out `x-protos` at that tag → `x-protos/scripts/generate.sh` (`buf.gen.yaml` lives in **x-protos**) → `go mod tidy`
- Commits on diff → tag + GitHub Release using the **same** version as proto (no patch auto-bump)

Variable: `PROTO_REPO` (default `huponx/x-protos`). A public proto repo needs no extra read token.

Job outputs and summary: `proto_ref`, `tag`, `committed`, `skipped`, `release_url`.
